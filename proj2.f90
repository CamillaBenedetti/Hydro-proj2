...
MODULE DATA
    integer :: N, i
    real*8 :: pi,pc_to_cm,kpc_to_cm,year_to_seconds,kbol,mu,mp,msun,grav
    parameter (N=1001) 
    parameter(pi=3.141592)
    parameter(pc_to_cm=3.085d18)
    parameter(kpc_to_cm=1000.*pc_to_cm)
    parameter(year_to_seconds=3.156d7)
    ...
    real*8 :: d(N),e(N),v(N),P(N),s(N),tem(N),q(N),bri(N),ds,ent(N),tdyn(N) !density, internal energy, velocity, pressure, momentum, temperature, q??, brightness, entropy, dynamical time
    ...
    xmin=0.
    xmax=3500.*kpc_to_cm
    deltax=50.*pc_to_cm   !! first Delta_x
    ...
    do i=1,N
       write(20,1001)xa(i)/kpc_to_cm,xb(i)/kpc_to_cm,dxa(i)/kpc_to_cm,dxb(i)/kpc_to_cm
    enddo
    close(20)
    ...
    if (sdr==0) then  !! Cartesian - not used here !!
        ...
        do i=1, N-1
        dvl1a(i)=xa(i+1)-xa(i)   !! Note that is centeorange in xb(i)
        end do
        dvl1a(N)=dvl1a(N-1)
        do i=2, N
        dvl1b(i)=xb(i)-xb(i-1)  !! Note that it is centeorange in xa(i)
        end do
        dvl1b(1)=dvl1b(2)
    ...
    ! INITIAL CONDITIONS & PARAMETERS OF THE CLUSTER
     ...
     gam=5./3. !heat capacity ratio
     cv=kbol/((gam-1.)*mu*mp) !specific heat at constant volume
     ...
     rvir=2866.8*kpc_to_cm  !! Virial radius
     rs=449.66*kpc_to_cm    !! Scale radius for NFW halo (depends on mnfw!)
     conc=rvir/rs       !! concentration of NFW halo !! rs= Scale radius for NFW halo (depends on mnfw!)
     fc=log(1.+conc)-conc/(1.+conc)
    
     mstars=1.e12*msun   !! Stellar mass of the BCG
     reff=10.*kpc_to_cm      !! Effective radius
     ahern=reff/(1.+sqrt(2.)) !coeff for Hernquist eq
     ...
     if(xb(i).ge.100.*kpc_to_cm) rhost(i)=0.   !! stars truncated at 100 kpc
     zfest=0.8*zfesol   !! stellar Fe abundance, constant for now
     zfesn=0.744/1.4    !! this is y_Ia/M_ejecta
     
     open(15,file='rhost.dat') !write stellar density
     do i=2,N
        write(15,1009)xb(i)/kpc_to_cm,rhost(i)
     enddo
     close(15)
     ...
     tnow=13.7           !! Current time in Gyear_to_seconds
    
    !!  Find the grid point corresponding to rvir (it might be useful)
    
       do i=1,N
          if(xb(i).gt.rvir)then
             ivir=i-1
             goto11
          endif
       enddo
    11 continue
       print*,'ivir=',ivir,real(xa(i)/kpc_to_cm),real(rvir/kpc_to_cm)
    ...
    !! CALCULATE MASSES (AND GRAVITY)
     ...
     do i=3,N      !! notice: do loop starts with i=3
        ...
        yy=xa(i)/rs                               !! grid point/rs=Scale radius for NFW halo (depends on mnfw!)
    ...
    do i=1,N
       gtot(i)=ggal(i)+gdark(i)+gbh(i) !! total gravity includes DM, galaxy & BH
       mtot(i)=mgal(i)+mdark(i)+mbh
    enddo
    
    open(24,file='grv.dat')     !! write gravity and mass of: galaxy, bh, tot
    open(25,file='mass.dat')
    do i=3,N
       write(24,1005)xa(i)/kpc_to_cm,ggal(i),gdark(i),gbh(i),gtot(i)
       write(25,1005)xa(i)/kpc_to_cm,mgal(i)/msun,mdark(i)/msun,mbh/msun,mtot(i)/msun
    enddo
    close(24)
    close(25)
    ...
    enddo
     do i=3,N-1
       write(20,1006)xa(i)/kpc_to_cm,mgasin(i)/msun !write the initial gas mass
     enddo
    1006 format(5(1pe12.4))
    
    print*,'f_bar(rvir) = ',real(barfrac(ivir)) !! print the baryon fraction at virial radius
    
     ent=kbol*tem/(d/1.937d-24)**0.66666667   !! this is the initial entropy profile
    
     do i=2, N-1
        tcool(i)=max(e(i)/Cool(tem(i),d(i)),3.156e7)   !! cooling time: key quantity !Cool is a function, see below: input temperature and density, output Cool=n_e*n_i*Lambda=de/dt slide12
        gmed=0.5*(gtot(i)+gtot(i+1)) !average gravity between two grid points
        tdyn(i)=sqrt(2*xb(i)/gmed)  !! this is the dynamical time
     end do
     tcool(1)=tcool(2) !boundary
     tcool(N)=tcool(N-1)
     tdyn(1)=tdyn(2) 
     tdyn(N)=tdyn(N-1)
     ...
     open(20,file='briiniz.dat')   !! in this file writes also tcool
     do i=2,ivir+1  !!N-1
        bri(i)=0. !bri=brightness
        do nn=i,N-2
           epsilonx=0.
           if(tem(nn).ge.1.e6)epsilonx=Cool(tem(nn),d(nn)) !epsilonx = Cool = de/dt
           if(nn.eq.i)then
           ...
           bri(i)=bri(i)+epsilonx*ds !bri = integral of ne*ni*lambda*dV, in spherical ni*ne*lambda*4pir^2dr, slide 17
        enddo
        bri(i)=max(1.d-25,2.*bri(i))   !! "2.*" in order to count the other side
        write(20,1008)xb(i)/kpc_to_cm,bri(i),ent(i),&  
                      tdyn(i)/year_to_seconds,tcool(i)/year_to_seconds !briiniz.dat: initial surface bri, entropy, dyn time, cooling time
     enddo
     close(20)
    ...
    open(20,file='resiniz.dat')
      do i=3,N
         write(20,1000)xa(i)/kpc_to_cm,xb(i)/kpc_to_cm,d(i)/1.937d-24,v(i)/1.e5,&
              tem(i),p(i),tcool(i)/year_to_seconds !2 grids, density, temperature, pressure, cooling time
      enddo
    close(20)
    
    open(20,file='abundiniz.dat')
      do i=2,N
         write(20,1010)xb(i)/kpc_to_cm,dfe(i),zfe(i)/zfesol !write initial abundance
      enddo
    close(20)
    ...
    !!!! Define some more parameters and start the time integration !!!!
     ...
     tmax=5.e9*year_to_seconds  !! final time of the simulation 
     tshift=8.7*(1.e9*year_to_seconds)  !! this is for the stellar source term
     ncicli=0
     ncont=0
     ncont2=0
    ...
    !! SUBSTEP 0: STELLAR TERMS, IRON DENSITY, SN ENERGY...
       ...
       tgyr=(t+tshift)/(1.e9*year_to_seconds)   !! to simulate a stellar pop of initial age "tshift"
       alphast=4.7e-20*(tgyr/tnow)**(-1.26)   !! new alphast for this cycle, alpha star time dependent
    !!   alphast=0.         !! if you don't want stellar winds
    
       alphasn=4.436e-20*(snu/aml)*(tgyr/tnow)**(-slope) !alpha supernovae time dependent
    !!   alphasn=0.         !! if you don't want SNIa
     ...
     do i=2,N-1
        ...
        dfe(i)=dfe(i)+dtmin*rhost(i)*(alphast*zfest(i)/1.4+alphasn*zfesn) !! add Fe !!
    ...
    !! SUBSTEP I: UPDATE OF VELOCITY FOR PRESSURE GRADIENT
    ...
    !! SUBSTEP II: UPDATE FOR THE ARTIFICIAL VISCOSITY
    ...
    !! SUBSTEP III: UPDATE FOR COMPRESSIONAL HEATING
        do i=2,N-1
    ...
    !! TRANSPORT STEP
        ...
        do i=2, N-1
            s(i)=0.5*(d(i)+d(i-1))*v(i)  !! this is at "i" !! s = momentum
        end do	
    ...
    !! UPDATE OF THE DENSITY, IRON DENSITY AND ENERGY
         ...
         !! CALCULATE MASS AND IRON MASS FLUXES
            do i=2, N-1
            if (v(i)>0.) then
                dstar(i)=d(i-1)     !! at i !! star density?
                dfestar(i)=dfe(i-1)     !! at i !! density of Fe prom stars
            else
                dstar(i)=d(i)
                dfestar(i)=dfe(i)
            end if
        end do
        dstar(N)=dstar(N-1) !boundary
        dstar(1)=dstar(3)
        dfestar(N)=dfestar(N-1)
        dfestar(1)=dfestar(3)
    
        do i=2, N
            F1(i)=dstar(i)*v(i)*g2a(i)*g31a(i)    !! at i !! is this the force?
        ...
        do i=2, N-1
            if (v(i)>0.) then
                e_dstar(i)=e(i-1)/d(i-1)   !! at i !! !energy/density?
            else
                e_dstar(i)=e(i)/d(i)
            end if
        end do
        e_dstar(N)=e_dstar(N-1) !boundary
        e_dstar(1)=e_dstar(3)
    ...
    !! UPDATE OF THE MOMENTUM
        ...
        do i=2, N-1
            s(i)=s(i)-dtmin/dvl1b(i)*(F3(i)-F3(i-1)) !momentum
        end do
    ...
    !!!!  Here the hydro update ends  !!!!
     ...
     if(t.ge.(ncont+1)*deltat)then  !! write results every time interval deltat
        ...
        do i=3,N     !! starts from 3 !!
           write(20,1000)xa(i)/kpc_to_cm,xb(i)/kpc_to_cm,d(i)/1.937d-24,v(i)/1.e5, &
                         tem(i),p(i),tcool(i)/year_to_seconds !datafile "clu00**": 1-2 grids, 3 density, 4 velocity, 5 temperature, 6 pressure, 7 cooling time
        end do
        1000 format(7(1pe11.3))
        close(20)
    
        do i=2,N
            write(21,1010)xb(i)/kpc_to_cm,dfe(i),zfe(i)/zfesol !datafile "abu00**: 2 Fe density, 3 Fe abundance
        enddo
        close(21)
    ...
    !! Calculate the X-ray surface brightness and other stuff
     ...
     do i=2,ivir+1   !! X-ray brightness within rvir
        ...
        write(42,1008)xb(i)/kpc_to_cm,bri(i),ent(i),&
                      tdyn(i)/year_to_seconds,tcool(i)/year_to_seconds !datafile "bri00**": 2 surface brightness, 3 dynamical time, 4 cooling time 
     enddo
    ...
    !! Calculate the masses of hot gass, cold gas...
        ...
        do i=2,N-1
           mcoldadd=0.
           mhotadd=0.
           if(tem(i).le.tcold) mcoldadd=d(i)*vol(i) !add mass as cold gas in the gas temp <= than tcold
           mcold(i)=mcold(i-1)+mcoldadd
           if(tem(i).gt.tcold) mhotadd=d(i)*vol(i) !add mass as hot gas in the gas temp > than tcold
    ...
    !!  Here is a new (better) fit to Lambda(T,Z):
          ...
          tlog=log10(tem1)
          if(tlog .ge. 6.1) then
             qlog0 = -26.39 + .471*(log10(tem1 + 3.1623e6))
          else if(tlog .ge. 4.9) then
             arg = 10.**(-(tlog-4.9)/.5) + .077302
